...
SENTENTENCE_TRANSFORMER_BATCH_SIZE = 64
...
class RAGModel:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        responses = self.llm.generate(
            ...
            vllm.SamplingParams(
                ...
                max_tokens=3200,  # Maximum number of tokens to generate per output sequence.
                stop=terminators,
        ...
        formatted_prompts = self.format_prompts(
            queries, query_times, message_no=4, context=clues_list
        )
        ...
        for response in responses:
            eval_list.append(response.outputs[0].text)
        ...
        formatted_prompts = self.format_prompts(
            queries, query_times, message_no=1, context=eval_list
        )
        responses = self.llm.generate(
            formatted_prompts,
            vllm.SamplingParams(
                n=3,  # Number of output sequences to return for each prompt.
                top_p=0.9,  # Float that controls the cumulative probability of the top tokens to consider.
    ...
    def format_prompts(
        self, queries, query_times, message_no, batch_retrieval_results=[], context=None
    ):
        """
        ...
        Return the well-reasoned and detailed clues and be concise and precise, do not go into long explanation if you dont have the answer. Only explain when the answer is present in the context."""

        messages_answer_majority = """You are an expert at finding the most relevant pieces of texts for anything that a user may ask.
      You will be provided a question that the user wants answered and also the clues to answer the question along with an evaluation of the clues.

      If the total evaluation score of the clues is less than 20/20, then you must only respond with I don't know the answer to this question as the clues are poor in quality.

      If any of the clues has a evaluation score of 20/20 then your job is to read all clues and pick only the clues that confidently answer the question. Pick the majority opinion from them as the answer to the question that the user requires.
      The answer should be based on the majority opinion from the confident clues provided, DO NOT ADD YOUR OWN INFORMATION NEITHER USE EXTERNAL SOURCES.

      If any of the clues has a evaluation score of 20/20 then you dont need to be super critical of the clues. You may consider it reliable source of information.
      ...
      Only if you are confident about the answer being correct, you will return the majority answer in detail at the very start. 
      If there are contradictory answers from the clues, the return 'I don't know'
      If you are absolutely sure that the answer does not exist in any of the clues or you have a hint that the answer may be incorrect, then reply with only... 'I dont know'.
      
      You will end with:
      ###Final Answer: <your answer>
       """

        messages_eval = """You will be given a question and references from the user, and a set of clues extracted by a model using the references. Your task is to evaluate the quality of the extracted clues based on the provided criteria.

Evaluate the clues using the criteria below, with a maximum score of 20 points:
- Relevance (2 points): Are the clues directly relevant to the user's question?
- Completeness (5 points): Do the clues cover all necessary information present in the context to address the user's question?
- Clarity (2 points): Are the clues clearly and concisely presented?
- Accuracy (4 points): Are the clues accurate based solely on the provided context document, without adding any external knowledge?
- Overall Relevance to Answer (5 points): How relevant are the clues in completely answering the user's question based on the provided context?

### Example 1 (High Score)
Here are the question and references:

**User Question:** What were the main contributions of Ada Lovelace to computing?

**References:**
* Ada Lovelace wrote the first algorithm intended for implementation on a computer.
* She was an analyst of Charles Babbage's early mechanical general-purpose computer, the Analytical Engine.
* Lovelace's notes on the Analytical Engine include what is recognized as the first algorithm meant to be carried out by a machine.
* She is often considered the first computer programmer.
* Ada Lovelace foresaw that computers could be used to manipulate symbols and create music.

**Clues:**
* Ada Lovelace wrote the first algorithm intended for implementation on a computer.
* She was an analyst of Charles Babbage's Analytical Engine.
* Lovelace's notes include what is recognized as the first algorithm meant to be carried out by a machine.
* She is considered the first computer programmer.

**Evaluation:**
- Relevance (3 points): The clues are directly relevant to the user's question. (3/3)
- Completeness (5 points): The clues cover all necessary information present in the context to address the user's question. (5/5)
- Sufficiency (5 points): The clues are sufficient to answer the user's question completely. (5/5)
- Coherence (3 points): The clues logically flow together and provide a coherent answer. (3/3)
- Clarity (2 points): The clues are clearly and concisely presented. (2/2)
- Accuracy (2 points): The clues are accurate based solely on the provided context. (2/2)
**Total Score**: 20/20

### Example 2 (Low Score)
Here are the question and references:

**User Question:** What are the key achievements of the Apollo 11 mission?

**References:**
* Apollo 11 was the first manned mission to land on the Moon.
* The mission took place in July 1969.
* Neil Armstrong and Buzz Aldrin were the first and second humans to walk on the Moon.
* Michael Collins piloted the command module alone in lunar orbit.
* The mission returned safely to Earth on July 24, 1969.

**Clues:**
* Apollo 11 was a mission.
* It took place in July.
* Neil Armstrong walked on the Moon.
* The mission returned.

**Evaluation:**
- Relevance (2 points): The clues are somewhat relevant to the user's question but lack specificity. (2/3)
- Completeness (2 points): The clues do not cover all necessary information. Important details are missing. (2/5)
- Sufficiency (1 point): The clues are not sufficient to fully answer the user's question. (1/5)
- Coherence (2 points): The clues do not fully logically flow together, and some information seems disjointed. (2/3)
- Clarity (1 point): The clues are not very clear and concise. (1/2)
- Accuracy (2 points): The clues are accurate but incomplete based on the provided context. (2/2)
**Total Score**: 10/20"""

        messages_self_con = """You will be given a question and 3 detailed answers for the question. 
      Your job is to look at the "Final Answer" section of the 3 detailed answers, judge the confidence of the answers and pick the common answer amongst only the confident answers as the true final answer.
      ...
      If there is no conclusive final answer in the detailed answer, then you will simply return 'I don't know.' as well.
      If you are not confident about the answer from the the detailed answers, then also return 'I don't know'.
      You do not need to explain why you chose an answer."""
        ...
        for _idx, query in enumerate(queries):
                ...
                formatted_prompts.append(
                    self.tokenizer.apply_chat_template(
                        [
                            ...
                            {
                                "role": "user",
                                "content": f"""<References>{references}<\References>""",
                            },
                            {
                                "role": "user",
                                "content": f"""Using the above references, provide clues for the following question: 
                          <User Question>{query}<User Question>
                          You will first repeat the question, then the references listed, and only then provide the clues for it...""",
                            },
                        ],
                        tokenize=False,
            ...
            if message_no == 4:
              ...
              formatted_prompts.append(
                  self.tokenizer.apply_chat_template(
                      [
                          {"role": "system", "content": messages_eval},
                          {"role": "user", "content": f"""<Clues>{clues}<\Clues>"""},
                          {
                              "role": "user",
                              "content": f""" 
                        <User Question>{query}<User Question>
              You will first repeat the question, then only the clues (not references), then give a total score and a brief justification for each criterion.""",
                          },
                      ],
                      tokenize=False,
                      add_generation_prompt=True,
                    ...
                    self.tokenizer.apply_chat_template(
                        [
                            ...
                            {"role": "user", "content": f"""<Clues>{clues}<\Clues>"""},
                            {
                                ...
                                "content": f""" 
                          <User Question>{query}<User Question>
                          You will first repeat the question, then only the clues (not references), then give a detailed explanation for your answer...""",
                            },
                        ],
                        tokenize=False,
                        add_generation_prompt=True,
                ...
                formatted_prompts.append(
                    self.tokenizer.apply_chat_template(
                        [
                            ...
                            {
                                "role": "user",
                                "content": f"""<Detailed Answers>{det_answers}<\Detailed Answers>""",
                            },
                            {
                                "role": "user",
                                "content": f""" <User Question>{query}<User Question>""",
                            },
                        ],
                        tokenize=False,
                        add_generation_prompt=True,

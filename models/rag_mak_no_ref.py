...
class RAGModel:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        formatted_prompts = self.format_prompts(
            queries, query_times, message_no=1, context=clues_list
        )
        responses = self.llm.generate(
            formatted_prompts,
            vllm.SamplingParams(
    ...
    def format_prompts(
        ...
        Return the well-reasoned and detailed clues alongwith the confidence you have in gathered clues from the provided context document regarding the query. 
      ...
      Your job is to read all clues and pick only the clues that confidently answer the question. Pick the majority opinion from them as the answer to the question that the user requires.
